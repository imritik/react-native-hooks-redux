        ...
        AsyncStorage.getItem('quotes', (err, quotes) => {
            if (err) alert(err.message);
                ...
                if (!edit) {
                    ...
                    //if quote is in the array,replace the quote
            ...
            <SafeAreaView style={styles.flex}>
                ...
                <View style={styles.buttonContainer}>
                    <View style={{ flex: 1, justifyContent: "center" }}>
                        <Text style={[styles.count, (MAX_LENGTH - text.length <= 10) && { color: "red" }]}>
                            {MAX_LENGTH - text.length}
                        </Text>
                    </View>
                    <View style={{ flex: 1, alignItems: "flex-end" }}>
                        <TouchableHighlight style={[styles.button]} disabled={disabled} onPress={onSave} underlayColor="rgba(0,0,0,0)">
                            <Text style={[styles.buttonText, { color: disabled ? "rgba(255,255,255,.5)" : "#FFF" }]}>Save</Text>
                        </TouchableHighlight>
                    </View>
                </View>
            </SafeAreaView>
